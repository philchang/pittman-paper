...
2008), coupled with a fast-Fourier transform solution on the root grid. Similar to (Lee et. al Feb. 2015) our FLASH runs use the Harten-Lax-van Leer-Contact Riemann solver and an unsplit solver. ({\bf Lee et. al use Lee \& Deane (2009) do we?})

We start with a box with the physical length set to $L = 16$pc using periodic boundary condtions. The intial mass density is $\rho = 3x10^{-22}$ and the isothermal, ambient gas is set to a sound speed of $c_s = 2.64x10^{4}$ {\bf we also set $Mach_ambient = 5.0$ include?}. To initialize our simulations, we drive turbulence by applying a large scale ($1 \le kL \le 2$) solenoidal acceleration field as a momentum and energy source term. We apply this field in the absence of gravity and sink particle formation for 3 dynamical times until a statistical steady state is reached.
...
\begin{equation}
%\frac{\rho}{\rho_0} = 45 \text{x} 4^{l} (\frac{N_{root}}{128})^2 (\frac{16\text{pc}}{L})^2 (\frac{c_s}{2.65x10^4 \text{cm s})^2 (\frac{3x10^{-22}\text{g cm^-3}}{\rho_0})
